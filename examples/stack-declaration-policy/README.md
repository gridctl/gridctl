...
`stack.yaml` is the candidate. `policy.yaml` is the operator-selected rule list. In CI, treat the checker binary, workflow, and policy as independently trusted from pull-request YAML. `ci-workflow.yml` is a design sketch, not a required repository check.
