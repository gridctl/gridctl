...
so it lands on that scratch. Persistent data uses `/data` with an explicit
volume; do not bind-mount over `/app` or the virtualenv.

Tested matrix (native execution, not a multiarch manifest): Linux amd64 and
arm64, Docker on GitHub-hosted runners. Podman is covered when the
integration suite runs on that runtime. Untested combinations are unsupported.
