...
- `gridctl agent init --lang ts|go` and `gridctl agent init --prompt-only`. The TS branch (default) produces the existing three-file scaffold unchanged; `--prompt-only` writes a single `SKILL.md` (no `skill.ts`, no `skill.go`, no `agent.json`) so authors can hand-edit a prompt body that surfaces to upstream MCP clients verbatim. `--prompt-only` is mutually exclusive with `--lang` — the pre-flight error fires before any files are written. The `--lang go` branch returns "not yet implemented" today; Phase 2 of the three-flavor skill authoring surface lands the Go scaffold body. Post-scaffold hint differentiates: TS prints `gridctl agent dev --root <dir>`; prompt-only prints `gridctl skill list --remote && gridctl run <name>`. `pkg/agent/dev/scaffold/scaffold.go` gains `Options.Language` (`""`/`"ts"` → TS default, `"prompt"` → SKILL.md only, `"go"` → Phase 2 stub error); `starterFiles` branches on the flavor; new `promptSkillMD(name)` renders the prompt-only body with active state so `store.Load()` picks it up.
