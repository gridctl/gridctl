...
- On the stateless generation, `resources/read` results now carry gateway-owned cache metadata (`ttlMs` 60000, private) instead of the downstream fleet's aggregate. Skill resources are served from the local registry, so their cache lifetime is unrelated to the tool fleet: previously one legacy tool server in the stack pinned every skill read to uncacheable (#1086)
