...
- `gridctl activate <skill-name>` CLI wrapping `POST /api/registry/skills/{name}/activate`. Restores parity with `AGENTS.md`, which already references the top-level command; previously operators promoting a skill from `draft` to `active` had to `curl` the API directly. Single positional argument, default port resolution mirroring `gridctl test` / `gridctl optimize`, exit codes `0` (activated) / `1` (skill not found) / `2` (infrastructure: gateway unreachable, conflict, registry unavailable, network failure), and Article X compliance via `--format json` (emits `{"skill":"<name>","state":"active"}`) and `--quiet` (suppresses the human-readable success line). Conflict and 5xx responses surface the daemon's error message verbatim so the operator sees the pre-condition that failed instead of a re-derived one.
