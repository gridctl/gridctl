...
- Python source containers now have complete CLI support: `gridctl plan` resolves first-class build actions with immutable source identity, image tag, and cache state, while `--show-dockerfile` exposes the exact generated file in text and JSON output. Apply and reload emit structured per-server build, startup, and connection phases. `gridctl add --container` writes exact structured PyPI or commit-pinned GitHub sources without changing the default host `uvx` catalog path, and `gridctl doctor` warns when host `uvx` is unavailable without requiring it for container workflows. The schema reference, CLI reference, troubleshooting guidance, and copy-paste PyPI example cover the new workflow and its security limits.
