...
- Add opt-in `gridctl validate --policy` for offline evaluation of captured stack declarations against a versioned six-rule policy file. Flag absence preserves ordinary validation; an explicit empty, missing, or invalid policy fails without falling back. The evaluator reads only the selected policy, entry stack, and bounded local extends under the candidate root, without environment, secret, registry, or runtime resolution. Acceptance is not deployment admission. Empty selections and all-N/A results are errors. Unknowns stay distinct from violations. Source-built servers remain excluded from digest claims. Literal `*` tool names warn without becoming globs. No API, UI, or runtime enforcement is added (#1226).
...
- Document offline stack declaration policy: source-root limits, unknown and N/A handling, declaration-versus-runtime scope, warning exit handling, trusted checker/policy selection, and a CI workflow design that does not execute pull-request code as the checker (#1226).
