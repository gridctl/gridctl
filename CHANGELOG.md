...
- Source image builds now resolve Git refs to full commits in isolated builder worktrees and use content-addressed image tags instead of `latest`. Build identity includes source content, Dockerfile selection, build arguments, command, and target platform, while config plan and hot reload share complete effective server and source comparison.
