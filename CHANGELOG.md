...
- Add opt-in metadata-only persisted run records for returning tool-dispatch attempts, including early denials. Recording uses a bounded queue and single writer, independent of tracing and metrics. Query through `gridctl runs`, `GET /api/runs`, and the Runs tab beside Traces. Default retention is seven days and 100 MiB per stack. Wipe is stack-wide and is not secure erasure. Recording is best-effort; names and caller-declared labels may still be sensitive (#1155).
