...
A pack is a git repository carrying a `gridctl-pack.yaml` manifest at its root: a versioned selector over the repo's skills, agents, and gateway wiring, so one import configures a whole team setup. Packs are a thin composition layer — `pack add` runs the same origin pipeline as `gridctl skill add` (security scan, `--trust` gate, drift-safe updates), and `pack apply` drives the same projection engines as `gridctl skill project sync` and `gridctl project sync --kind wiring`, scoped to the pack. Every projection a pack applies is tagged with the pack name in `~/.gridctl/project.lock.yaml`, which is what makes `pack status` and cascade removal exact.
...
rules: []                            # reserved; parsed but not yet applied
```

Skills follow the `SKILL.md` convention and agents the `agents/*.md` convention, exactly as plain skill repos do — a pack repo is a skill repo plus a manifest. Names the manifest selects but the repo does not ship are reported as `unresolved` (exit 1) and kept in the pack record so status stays honest.
...
| `gridctl pack remove <name>` | Cascade removal in dependency order: projections unsynced from client trees, wiring records removed through the ownership manager (entries gridctl did not record are never deleted), then the pack's registry skills and agents, then the pack record. Drifted projections are kept with a remediation hint unless `--force`; a partial removal trims the pack record to what stayed. `--dry-run`, `--format json`. |
