...
Authentication lifecycle evidence was updated on September 12, 2026, against source revision `b903bf373480cd7d3cb4328166da17de09c59ddf`. The lifecycle controls below are implemented in this checkout and Unreleased; this is not evidence of merge or publication. Release claims retain the original baseline.

Execution evidence was updated on September 12, 2026, against implementation commit `1465e1c` and documentation commit `8d4bd26`. These opt-in controls are implemented and Unreleased. Local Docker and process checks are recorded; required positive rootless Podman acceptance remains pending the hosted real-runtime gate. This does not establish cross-runtime acceptance, merge approval, or release readiness.
...
| Merged but unreleased | Changes after that tag include dependency and Actions updates, a read-only tap authentication diagnostic, and a macOS-only declarative replacement for the deprecated Homebrew quarantine-removal hook. Do not infer those changes are in the published archives. |
...
Gateway activity -> logs, metrics, traces, optional run records, and optional external exporters
...
| Runtime to observability and exports | Diagnostics and operational data | Disclosure through logs, traces, run records, copied stack files, or support bundles |
...
| Client and group exposure | `clients:` and `groups:` are opt-in. With no client policy, clients see all tools. A configured client policy denies unmatched identities unless `default: allow`; group membership and client scope are enforced for direct calls and code-mode inner calls. `gridctl call` / `POST /api/tools/call` and live discovery use the same scope and gates with default label `cli` and optional `--as` / `client`. | [Client scope](../../pkg/mcp/clientscope.go), [client identity extraction](../../pkg/mcp/clientid.go), [gateway dispatch](../../pkg/mcp/gateway.go), and [scope integration tests](../../tests/integration/per_client_scope_test.go) | Client identity comes from a query parameter, header, declared client name, or CLI `--as`, not a credential-bound principal. `--as` can select a broader configured profile than `cli` and does not authenticate as that client. Scope aliases do not share rate-limit buckets. These are exposure controls for cooperating clients, not tenant isolation. |
...
The browser verifies Bearer or API-key credentials against protected `/api/status` before replacing active state or persisting one versioned `gridctl-auth-credential` localStorage entry. API keys use raw `Authorization` by default or a configured browser-supported custom header. Legacy `gridctl-auth-token` text remains an opaque Bearer token when the new entry is absent. Origin scripts can read either entry; treat the origin, its scripts, extensions with access to it, and the workstation as credential-bearing surfaces. Storage failure permits verified current-window use with a visible re-entry notice; another window may still use its older saved credential. See [credential storage and header validation](../../web/src/lib/credentials.ts) and [browser guidance](../config-schema.md#browser-credentials).

The [shared browser request policy](../../web/src/lib/gatewayRequest.ts) restricts requests to the gateway origin, refuses redirects before following them, and validates credential headers against Fetch restrictions and request-owned fields. Middleware marks its own 401 denials with `Gridctl-Auth-Rejected: 1`; downstream 401s and other failures do not invalidate usable gateway credentials. Credential generations guard late responses, protected polling pauses during re-entry, and mutations are not automatically replayed. The main shell and all six detached routes share [AuthBoundary](../../web/src/components/auth/AuthBoundary.tsx). The legacy `/sse` fetch only consumes negotiation, not shutdown notification; polling detects disconnects. Evidence includes [request tests](../../web/src/__tests__/gatewayRequest.test.ts), [prompt tests](../../web/src/__tests__/AuthPrompt.test.tsx), and [real-browser acceptance](../../web/gateway-auth.browser.mjs).

Listener authentication, bind, Host/Origin allowlists, and the insecure override are startup-bound. The controller supplies an immutable effective startup snapshot to [reload security preflight](../../pkg/reload/security.go). Reload and stackless initialization compare resolved candidate values before no-op detection or application. `restart_required` rejects mixed edits without changing accepted/live state, while save-first operations retain authored YAML on disk. Failed resolution or validation returns value-free `invalid_candidate`. Observable stored-token rotation is compared with the original value, not a re-resolved old reference. [Security matrix tests](../../pkg/reload/security_test.go), [API tests](../../internal/api/reload_security_test.go), and [real restart tests](../../tests/integration/auth_restart_test.go) cover these boundaries. Follow [restart recovery](../troubleshooting.md#gateway-security-requires-a-restart) to activate new settings and end old process-owned sessions/streams. There is no live rotation or in-process revocation, and ordinary post-preflight reload failures retain their existing partial-application semantics.
...
| Local MCP processes | Omission retains ambient inheritance with the internal-credential denylist. Opt-in `execution.mode: local` defaults to no ambient inheritance and absolute executable lookup; explicit/scoped delivery retains precedence, and reserved keys remain denied. | [Process client](../../pkg/mcp/process.go), [execution tests](../../pkg/mcp/execution_test.go), and [lifecycle tests](../../pkg/mcp/process_lifecycle_test.go) | Local processes remain unsandboxed with the operator's filesystem/network authority. Absolute paths do not authenticate code; `ambient_path` explicitly uses gateway PATH independently of the child environment. Bootstrap receives no automatic relaxation. |
| Containers | Omission retains compatibility. Opt-in `execution.mode: hardened` requires numeric non-root UID/GID, nonprivileged/private-PID execution, engine-default seccomp, and finite per-replica memory/CPU/PID limits. Defaults also require read-only root, NNP, all capabilities dropped, bounded scratch, and network none. | [Contract resolver](../../pkg/execution/config.go), [engine admission](../../pkg/runtime/docker/execution.go), [Linux observer](../../pkg/runtime/docker/execution_linux.go), and [real-runtime tests](../../tests/integration/execution_test.go) | Required unavailable or mismatched evidence blocks routing. Local Linux instance-bound observations are required; API compatibility, image labels, and create success are insufficient. Explicit exceptions remain visible. Resources are not covered. Containers share a kernel and do not guarantee escape prevention. |
...
### Execution Evidence and Residual Authority

Hardened admission checks inspected controls before start where possible, then binds the daemon's full container ID and PID to local cgroup v2/kernel observations. The process may act before post-start verification; readiness gating does not undo those actions. Registration, health checks, dispatch, and raw relay recheck eligibility. A detected mismatch withdraws routing and stops the instance, with cleanup failures surfaced. These are snapshots, not continuous monitoring or protection against a runtime administrator changing controls between checks.

Execution transitions preflight before acceptance. Once accepted, superseded routes and replicas retire before replacement preparation; failure does not restore weaker execution. Execution-only recreation preserves schema pins. Static replicas, scaling, recovery, and idle wake-up carry the same contract, but each new instance needs its own evidence. See [reload acceptance tests](../../tests/integration/execution_daemon_test.go), [HTTP endpoint tests](../../tests/integration/execution_http_test.go), and [scaling tests](../../tests/integration/execution_scaling_test.go).

Read-only root does not bound persistent data volumes or prevent reading secrets in a declared mount. Hardened data mounts are plain engine-local volumes; host binds, engine sockets, device grants, plugins, and host-backed local-driver options are refused. Bounded nonexecutable scratch is charged to per-replica memory. Network none omits ports, managed attachments, and Gridctl's host alias; connected is an explicit exception without destination filtering or host isolation. Mounted sockets and MCP output remain authority and disclosure paths. Image pulls and build downloads are outside runtime networking controls. See [mount tests](../../tests/integration/execution_volumes_test.go) and [execution support guidance](../execution.md).

Selected local execution uses Unix process groups for ordinary descendant signaling and bounded direct-child cleanup. A detached descendant can survive, and stopping the local SSH client does not prove remote cleanup. SSH rejects execution declarations rather than claiming remote confinement. Process reports contain environment names and direct-child state, never environment values; container reports contain allowlisted observations, never raw inspect payloads or mount sources. See [signal tests](../../pkg/mcp/process_signals_test.go) and [report schema](../api-reference.md#execution-reports).
...
| Internal-credential boundary | `GRIDCTL_*`, `OP_CONNECT_TOKEN`, and `OP_SERVICE_ACCOUNT_TOKEN` are reserved. Store mutation, store-derived resolution, export/set delivery, and local MCP process inheritance apply this boundary. | [Reserved-key policy/tests](../../pkg/vault/internal_credentials_test.go), [policy implementation](../../pkg/vault/internal_credentials.go), and [process environment](../../pkg/mcp/process.go) | Ordinary non-reserved environment values are inherited when execution is omitted; the opt-in local inheritance allowlist narrows ambient delivery. Renaming a credential outside the reserved set changes whether the denylist recognizes it. This is not complete credential discovery. |
...
| Log redaction | The logging handler redacts recognized patterns and registered exact values. Verbose apply diagnostics use bounded summaries rather than dumping resolved configuration. | [Redaction implementation/tests](../../pkg/logging/redact_test.go) and [controller diagnostic tests](../../pkg/controller/export_diagnostic_test.go) | Unknown or transformed secrets, hostile output, raw files, and external consumers remain disclosure risks. Truncation limits size, not sensitivity. Logs, traces, and run records are operational evidence, not an immutable security audit trail. Run records omit payloads; names and caller-declared labels may still be sensitive. |
| Passive security evidence report | `gridctl doctor --security` and `GET /api/security-report` assemble existing snapshots without scanning, probing, or mutation. File and snapshot modes are offline. | [Assembler](../../pkg/secreport/assemble.go), [API handler](../../internal/api/security_report.go), and [report tests](../../pkg/secreport/assemble_test.go) | Exit zero is not a secure verdict. Allowlisted identifiers may still contain operator-authored secrets. Missing optional producers are unknown. Refresh does not re-observe downstream evidence. |
