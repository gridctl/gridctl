...
Read-only or permission failures require declared writable data/scratch and permissions appropriate for the selected UID/GID. Keep code and dependencies read-only. Do not use privileged mode, `chmod 777`, recursive code ownership changes, or automatic network access. `npx`/`uvx` may need explicitly supplied cache, HOME, or PATH settings; bootstrap gets no automatic relaxation. Local execution still has the operator's filesystem and network authority.

The [Python MCP runtime base](mcp-runtime-python.md) is a foundation image, not a ready server. Registry pull failures, missing execution evidence, and application errors are separate from permission failures.
