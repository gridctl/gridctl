...
The cask configuration uses GoReleaser's `custom_block` as a temporary bridge until the pinned generator supports declarative post-install steps natively. Remove the bridge only after a reviewed GoReleaser release can emit the same `postflight_steps` contract without restoring the deprecated hook. The exercise requires that contract and matching archive checksums. The release assembly applies the same validator and `ruby -c` to the actual generated cask before `release.py prepare` copies it byte-for-byte and records that digest in the checksum and provenance subject lists. These checks do not establish Homebrew behavior. Offline `test_*.py` discovery tests validator rejection cases, release ordering, and prepared-byte identity without invoking or downloading GoReleaser.

Homebrew 6.0.13 is the minimum supported version for this steps block. Before documenting that floor, the production-derived cask was loaded without the legacy warning on isolated Homebrew 6.0.13 and 7.0.6 installations. Disposable Homebrew 7.0.6 fixtures also covered macOS install and reinstall, binary execution, quarantine removal, propagated xattr failure, and Linux installation with `/usr/bin/xattr` absent. Changes to the steps block or version floor require this real Homebrew load and installation acceptance again, with exact Homebrew versions, operating systems, and architectures recorded. These local fixtures do not establish public asset availability, provenance identity, or byte-identical production tap publication. Those remain authorized release-workflow gates.
...
For a tap authentication failure, the separate `Tap authentication diagnostic` workflow can be manually run on production `main` after review and explicit approval. It uses the existing tap secret for GET requests to `gridctl/homebrew-tap/contents/Casks/gridctl.rb` through the release Python helper and the pinned GitHub CLI. It reports HTTP status, fixed error categories, and boolean credential-format checks; credentials, response bodies, and CLI error text are not printed. A failed probe makes the diagnostic job fail. A successful read does not prove write access. The diagnostic does not publish, update the tap, or retry release jobs. Unknown errors remain generic; a credential replacement is not inferred from a 401 alone.
