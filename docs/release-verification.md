...
Homebrew 6.0.13 is the minimum supported version for this steps block. Before documenting that floor, the production-derived cask was loaded without the legacy warning on isolated Homebrew 6.0.13 and 7.0.6 installations. Disposable Homebrew 7.0.6 fixtures also covered macOS install and reinstall, binary execution, quarantine removal, propagated xattr failure, and Linux installation with `/usr/bin/xattr` absent. These local fixtures do not establish public asset availability, provenance identity, or byte-identical production tap publication. Those remain authorized release-workflow gates.
