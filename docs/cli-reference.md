...
| `gridctl validate <stack.yaml>` | Validate stack YAML (exit `0`/`1`/`2`); `--format json` or `--json` for machine-readable output. With a `model_preferences:` block present, also emits the advisory findings `model-preference-unknown-alias`, `model-preference-unhonored`, and `model-preference-portability` (warnings only, never blocking). `--check-mutable-refs` (default off) adds local advisory findings for mutable image tags and unpinned `npx`/`uvx` selectors; informational `reference-not-assessed` findings and a coverage note appear even when there are no warnings. Exit zero with those info findings is not complete coverage. `--policy <policy.yaml>` evaluates captured declarations against a versioned offline rule file without environment, secret, registry, or runtime resolution; flag absence preserves ordinary validation, and an explicit empty or invalid policy does not fall back. REST validation, health counts, apply, and stack schema are unchanged. |
| `gridctl plan <stack.yaml>` | Preview changes against running state with Terraform-style colored `+`/`~`/`-` symbols. Source servers are resolved to immutable build actions showing declared and resolved identity, the content-addressed image tag, mutable-ref warning, and cache state (`cached`, `build`, or `unknown` when no runtime is reachable). `--show-dockerfile` includes the exact generated Python Dockerfile in text output and each JSON build action; without it, Dockerfile text is omitted. MCP server comparison covers the complete effective configuration, including source auth, build arguments, commands, volume mounts, replica settings, autoscaling policy, and execution controls. Equivalent normalized defaults compare equal; execution preserves meaningful empty/false intent (see [Execution Results](#execution-results)). `-y` / `--auto-approve` to apply, `--format json` or `--json` for machine output. |
...
| `gridctl reload [stack-name]` | Hot reload a running stack's resolved spec (accepts a stack name or file path; omit for all running stacks). `--format text\|json` (default `text`); manual reload does not require `--watch`. Effective gateway security changes reject the whole reload with `restart_required`. |
...
```bash
gridctl validate stack.yaml --check-mutable-refs --format json
```
...
`gridctl validate stack.yaml --format json` includes an optional `execution` map keyed by server name. Its normalized requested controls are `configured`, the report is `pending`, and `eligible` is false. Validation does not contact the runtime to establish enforcement; exit codes remain `0` for valid without warnings, `1` for validation errors, and `2` for warnings only. File or parsing failures return a command error.

`gridctl plan stack.yaml` reports `execution changed: recreate; review resolved restrictions and relaxations` when the effective contract changes, including removal. Execution lists preserve explicit empty intent: `tmpfs: []` differs from omitted default scratch. Equivalent normalized defaults compare equal.

`gridctl status --json` retains optional server and replica `execution` reports. `gridctl status --replicas` appends `execution <outcome> (<mode>)` to the existing STATE cell; this is separate from MCP health. Local process reports say `unsandboxed-local`; SSH reports say `external-ssh; remote cleanup unverified`. A false execution eligibility value on those process reports does not itself block process routing or mean an OS sandbox exists. Hardened containers require eligible evidence before dispatch. See the [report schema](api-reference.md#execution-reports).

The additional metadata and changed human replica output are Unreleased breaking output changes under Article VIII, requiring maintainer-owned major-release scheduling. Migrate scripts that parse STATE text to JSON, accept optional execution fields, and inspect each replica rather than interpreting healthy MCP state as enforcement.

### Reload Results

`gridctl reload [stack-name] --format json` emits newline-delimited JSON, one object per returned reload result, with `stack`, `success`, and `message`. Nonempty `added`, `removed`, `modified`, and `errors` lists are included when present. Preflight rejection adds `code`: `restart_required` includes `changed_fields` containing field names only; `invalid_candidate` omits that list. Unlike the REST preflight response, CLI JSON has no nested `error` object. This command accepts `--format text|json`, not the table-oriented `--json` alias.

Exit `0` means all attempted reloads succeeded, including no changes or no running stacks. Exit `1` covers a rejected/failed reload, invalid format, stack lookup failure, or transport/output error. Without a stack argument, every running stack is attempted even if one fails; the command exits `1` if any failed. No running stacks produces no JSON records. In JSON mode, human errors go to stderr; a transport failure has no returned reload-result record.

```bash
gridctl reload --format json
gridctl reload gateway-remote --format json
```

An effective startup-security change rejects security-only and mixed edits before application, even when an unchanged variable reference resolves to a rotated stored value. The saved YAML and active credential remain unchanged by rejection. Follow [gateway restart recovery](troubleshooting.md#gateway-security-requires-a-restart); repeated reloads cannot activate it. Restore security-equivalent settings if you want to resume ordinary reloads without restarting.

MCP execution changes are reloadable after their own capability preflight. Once accepted, old routes and replicas retire before replacement; failed preparation does not restore a weaker contract. Execution-only recreation preserves schema pins, even with simultaneous replica/autoscale tuning. See [execution recovery](troubleshooting.md#mcp-execution-admission).
...
| `gridctl doctor` | Run opinionated environment checks with remediation hints: the active home directory and its source (`GRIDCTL_HOME` / `--home` / default), runtime detection, socket reachability, version floor, gateway port, `npx` and `uvx` availability, state directory hygiene, stale state files, vault status, the projection lockfile, client wiring state, and per-server MCP protocol generation (flagging mixed-generation fleets). Missing `uvx` is an advisory warning for host PyPI workflows; generated Python containers do not require host Python or uv. `--json` for a machine-readable report, `-q` to print only failures and warnings. Ordinary exit `0` (no errors; warnings allowed), `1` (errors), `2` (doctor failed). `--security` emits a separate [passive security evidence report](security-evidence.md) and does not run those probes; `--source file:<path>\|snapshot:<path>\|gateway:<base-url>` is required and has no fallback. `--source` is rejected outside `--security`. Security quiet output keeps failures, warnings, unknowns, source identity, and coverage; `--json -q` still emits the full `gridctl.security-report.v1` DTO. Security exit `0` (no established failures among documented fail predicates; warnings and unknowns allowed; not a secure verdict), `1` (documented fail predicates), `2` (source or report failure). |
