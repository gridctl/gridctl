...
| Command | Purpose |
|---|---|
| `gridctl pack add <repo-url>` | Import a pack's selection into the registry (`--ref`, `--trust`, `--dry-run`, `--format json` or `--json`). Exit `0` clean, `1` partial (unresolved or skipped), `2` infrastructure. |
...
| `gridctl skill project status` | Per-projection state table for skills and agents (in-sync / stale / drifted / target-missing; agent rows are marked experimental; `--stack <path>` to judge staleness against that stack's `model_preferences:` policy; `--format json` or `--json`, `--plain`; exit `0`/`1`/`2`). Rewritten projections show `copy (model policy)` as their channel and carry `model_value` in JSON. |
