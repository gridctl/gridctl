...
| `gridctl plan <stack.yaml>` | Preview changes against running state with Terraform-style colored `+`/`~`/`-` symbols. MCP server comparison covers the complete effective configuration, including source auth, build arguments, commands, replica settings, and autoscaling policy; omitted defaults and empty collections compare equal to their explicit equivalents. `-y` / `--auto-approve` to apply, `--format json` or `--json` for machine output. |
